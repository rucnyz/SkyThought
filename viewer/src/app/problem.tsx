...
const FileSelector = React.memo(
  ({ onSelect }: { onSelect: (files: string[]) => void }) => {
    const [datasets, setDatasets] = useState<string[]>([]);
    const [selectedFiles, setSelectedFiles] = React.useState<Selection>(
      new Set()
    );
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {currentData.map((item, index) => (
            <div key={index} className="p-4 rounded-2xl shadow-md bg-gray-50">
              <p className="font-bold text-lg mt-2">Metadata:</p>
              <pre>{item.metadata}</pre>

              <p className="font-bold text-lg">Problem:</p>
              <div className="bg-gray-100 border p-2">
                <MathJax inline style={{ whiteSpace: "pre-wrap" }}>
                  {item.problem}
                </MathJax>
              </div>

              <p className="font-bold text-lg mt-2">Response:</p>
              <div className="overflow-auto max-h-[80vh] bg-gray-100 border p-2">
                <MathJax inline style={{ whiteSpace: "pre-wrap" }}>
                  {item.response}
                </MathJax>
              </div>

              <p className="font-bold text-lg mt-2">Solution:</p>
              <div className="overflow-auto max-h-[80vh] bg-gray-100 border p-2">
                <MathJax inline style={{ whiteSpace: "pre-wrap" }}>
                  {item.solution}
                </MathJax>
              ...
              <p className="font-bold text-lg mt-2">Full Response:</p>
              <div className="overflow-auto max-h-[80vh] bg-gray-100 border p-2">
                <MathJax inline style={{ whiteSpace: "pre-wrap" }}>
                  {item.model_response}
                </MathJax>
